...
public class S3Config {
	...
	public AmazonS3 amazonS3() {
		...
		AmazonS3 amazonS3 = AmazonS3ClientBuilder.standard()
				.withRegion(property.getS3().getRegiao())
				.withCredentials(new AWSStaticCredentialsProvider(credenciais))
				.build();
		
		//Cria um bucket no S3 se o mesmo não existir
		if (!amazonS3.doesBucketExistV2(property.getS3().getBucket())) {
			...
			//Cria uma regra para excluir arquivos ainda não salvos no S3
			...
			//Configura a regra no bucket no S3
